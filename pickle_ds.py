...
if args.hierarchical:
    if not os.path.exists('data/hierarchical'):
        os.makedirs('data/hierarchical')
    parent_folder = 'data/hierarchical'
    
else:
    if not os.path.exists('data/standard'):
        os.makedirs('data/standard')    
    parent_folder = 'data/standard'
...
if not args.zero_shot:
    ...
    if data_set.granularity == 'mixed' or data_set.granularity == None:
        path = (parent_folder+'/dim(' + str(len(args.dimensions)) + ',' + str(args.dimensions[0]) + ')' + sample + '_sf' +
                str(args.scaling_factor) + '.ds')
    else:
        path = (parent_folder+'/dim(' + str(len(args.dimensions)) + ',' + str(args.dimensions[0]) + ')' + sample + '_granularity_'
    ...
    if args.zero_shot_test is None:
        for cond in ['generic', 'specific']:
            data_set = DataSet(args.dimensions,
                               ...
                               granularity=args.granularity)
            if data_set.granularity == 'mixed' or data_set.granularity == None:
                path = (parent_folder+'/dim(' + str(len(args.dimensions)) + ',' + str(args.dimensions[0]) + ')' + sample + '_' +
