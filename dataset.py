...
class DataSet(torch.utils.data.Dataset):
    ...
    #     """
    #     Returns all possible fixed vectors for a given dataset size.
    #     Fixed vectors are vectors of length len(properties_dim),
    #     where 1 denotes that an attribute is fixed, 0 that it isn't.
    #     """
    #     num_attributes = len(properties_dim)
    #     fixed_vectors = []

    #     # level = number of fixed attributes
    #     for level in range(1, num_attributes + 1):
    #         # choose which attributes are fixed
    #         for fixed_indices in itertools.combinations(range(num_attributes), level):
    #             vector = [0] * num_attributes
    #             for idx in fixed_indices:
    #                 vector[idx] = 1
    #             fixed_vectors.append(tuple(vector))
